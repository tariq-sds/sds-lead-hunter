    ...
    <div style={{ ...ST.card, borderLeft: `3px solid ${STAGE_COLORS[lead.stage] || C.border}` }}>
      ...
      >
        <div style={{ flex: 1, paddingRight: "10px" }}>
          ...
          <div style={{ fontSize: "12px", color: C.muted, marginTop: "3px" }}>
            {[lead.location, lead.type].filter(Boolean).join(" · ")}
          </div>
        </div>
      ...
      {expanded && !editing && (
        <div style={{ marginTop: "14px" }}>
          {lead.description && (
            <div style={{ fontSize: "13px", color: "#bbb", lineHeight: "1.6", marginBottom: "12px" }}>{lead.description}</div>
          )}
          {lead.notes && (
            <div style={{ fontSize: "12px", color: C.muted, fontStyle: "italic", marginBottom: "12px", padding: "8px 12px", background: C.surface, borderRadius: "6px" }}>
              {lead.notes}
            </div>
          )}
          <label style={ST.label}>Move stage</label>
          <select
            style={{ ...ST.select, marginBottom: "12px" }}
            value={lead.stage}
...
export default function SDSLeadHunter() {
  ...
  const [leads, setLeads] = useState(() => {
    try { return JSON.parse(localStorage.getItem("sds-leads-v3") || "[]"); } catch { return []; }
  });
  const [scanResults, setScanResults] = useState(() => {
    try { return JSON.parse(localStorage.getItem("sds-scan-results-v3") || "[]"); } catch { return []; }
  });
  ...
  const STAGE_COLORS = {
    "New": "#C9A84C", "Contacted": "#5B9BF0", "Proposal Sent": "#A78BFA",
    "Negotiating": "#F59E0B", "Won": "#4CAF7D", "Lost": "#555",
  };

  const C = {
    bg: "#080808", surface: "#111111", card: "#161616", border: "#242424",
    gold: "#C9A84C", goldLight: "#DDB96A", goldDim: "#C9A84C33",
    text: "#EDEDED", muted: "#777", dim: "#444", danger: "#B84040",
  };
  ...
  const ST = {
    ...
    btn: (v = "primary") => ({
      ...
      backgroundColor: v === "primary" ? C.gold : v === "danger" ? C.danger : "transparent",
      color: v === "primary" ? "#080808" : C.text, transition: "opacity 0.15s",
    }),
    badge: (color) => ({
      ...
      backgroundColor: color + "1A", color: color, border: `1px solid ${color}44`,
    }),
    input: {
      width: "100%", backgroundColor: C.surface, border: `1px solid ${C.border}`,
      borderRadius: "8px", padding: "11px 13px", color: C.text, fontSize: "14px",
      fontFamily: "Outfit, sans-serif", boxSizing: "border-box", outline: "none",
    },
    select: {
      width: "100%", backgroundColor: C.surface, border: `1px solid ${C.border}`,
      borderRadius: "8px", padding: "11px 13px", color: C.text, fontSize: "14px",
      fontFamily: "Outfit, sans-serif", boxSizing: "border-box", appearance: "none", cursor: "pointer",
    },
    label: {
      fontSize: "10px", color: C.muted, marginBottom: "6px", display: "block",
      textTransform: "uppercase", letterSpacing: "0.1em",
      fontFamily: "DM Mono, monospace", fontWeight: "500",
    },
    sectionLabel: {
      fontSize: "11px", fontWeight: "700", color: C.goldLight,
  ...
  const runScanner = async () => {
    setScanning(true);
    setScanLog("Searching London planning portals and hospitality press...");
    setScanResults([]);
    try {
      const data = await callClaude({
        ...
        system: `You are a lead intelligence scanner for Sonic Design Studios (SDS), a London-based luxury architectural audio consultancy.

Search for:
1. Recent planning applications (last 60 days) in London for restaurants, bars, hotels, nightclubs, members clubs, rooftop venues, private dining, event spaces — new builds and significant refurbishments.
2. Newly announced London hospitality openings not yet open.
3. Large-scale premium residential developments in London.

Return ONLY a valid raw JSON array. No markdown. No preamble.
Each item: { "name": string, "location": string, "borough": string, "type": string, "description": string, "ref": string, "relevance": "high"|"medium", "source": string }
Mark "high" for: large multi-zone venues, luxury/members club, hotel, premium residential.`,
        messages: [{ role: "user", content: "Search for London hospitality venue planning applications and new openings from the last 60 days. Include restaurants, hotels, bars, nightclubs, members clubs, premium residential. Return JSON array only." }]
      });
      ...
      if (match) {
        ...
        setScanLog("No structured results returned — try again.");
      }
    } catch {
      setScanLog("Scan failed. Check connection and retry.");
    }
    setScanning(false);
  };
  ...
  const addToPipeline = (result) => {
    ...
    saveLeads([{
      id: `l-${Date.now()}`, name: result.name, location: result.location,
      borough: result.borough, type: result.type, description: result.description,
      notes: "", stage: "New", sourceRef: result.ref,
      addedAt: new Date().toISOString(), source: "scanner",
    }, ...leads]);
  };
  ...
  const calcFees = () => {
    ...
    return {
      design: Math.round(base * 0.35), spec: Math.round(base * 0.25),
      install: Math.round(base * 0.28), commission: Math.round(base * 0.12),
...
Structure your response with these exact headings followed by a short paragraph each. No bullet points. UK English. Commercially sharp.
...
NEXT STEPS`,
      ...
      setBriefResult("Error interpreting brief. Please try again.");
    }
    setBriefing(false);
  };
    ...
    <div style={{ padding: "16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "16px" }}>
        <div>
          ...
          {lastScanned && (
            <div style={{ fontSize: "11px", color: C.muted, fontFamily: "DM Mono, monospace", marginTop: "-8px", marginBottom: "4px" }}>
              Last scan: {new Date(lastScanned).toLocaleDateString("en-GB", { day: "numeric", month: "short" })} {new Date(lastScanned).toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })}
            </div>
          )}
        </div>
      ...
      {scanning && (
        <div style={{ textAlign: "center", padding: "56px 20px" }}>
          ...
          <div style={{ fontSize: "13px", color: C.muted }}>Searching London planning portals and hospitality press...</div>
        </div>
      )}
      ...
      {!scanning && scanResults.length === 0 && (
        <div style={{ textAlign: "center", padding: "56px 20px" }}>
          ...
          <div style={{ fontSize: "13px", color: C.muted }}>Tap Run Scan to search for live London hospitality opportunities</div>
        </div>
      )}
        ...
        return (
          <div key={r._id} style={{ ...ST.card, borderLeft: `3px solid ${r.relevance === "high" ? C.gold : C.dim}` }}>
            ...
            <div style={{ fontSize: "11px", color: C.muted, fontFamily: "DM Mono, monospace", marginBottom: "8px" }}>
              {[r.location, r.borough, r.type].filter(Boolean).join("  ·  ")}
            </div>
            ...
            <button
              style={{ ...ST.btn("ghost"), fontSize: "12px", padding: "7px 14px", opacity: inPipeline ? 0.45 : 1 }}
              onClick={() => !inPipeline && addToPipeline(r)}
              disabled={inPipeline}
            >
  ...
  const renderPipeline = () => (
    <div style={{ padding: "16px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
        ...
        <button style={{ ...ST.btn("ghost"), fontSize: "12px", padding: "7px 13px" }} onClick={() => {
          saveLeads([{ id: `l-${Date.now()}`, name: "New Lead", location: "", type: "", description: "", notes: "", stage: "New", addedAt: new Date().toISOString(), source: "manual" }, ...leads]);
        }}>+ Add</button>
      </div>
        ...
        {["All", ...STAGES].map(s => {
          ...
          return (
            <button key={s} onClick={() => setStageFilter(s)} style={{
              ...
              fontSize: "11px", fontFamily: "Outfit, sans-serif", fontWeight: "600", whiteSpace: "nowrap",
              backgroundColor: stageFilter === s ? C.gold : C.surface,
              color: stageFilter === s ? "#080808" : C.muted, transition: "all 0.15s",
            }}>
              {s}{cnt > 0 ? ` (${cnt})` : ""}
            </button>
          );
    ...
    <div style={{ padding: "16px" }}>
      ...
      {fees && (
        <div style={{ ...ST.card, borderColor: `${C.gold}44` }}>
          ...
          {[
            { label: "Design & Consultancy", value: fees.design },
            { label: "Specification", value: fees.spec },
            { label: "Installation Oversight", value: fees.install },
            { label: "Commissioning", value: fees.commission },
          ].map(row => (
  ...
  const renderBrief = () => (
    <div style={{ padding: "16px" }}>
      ...
      <div style={ST.card}>
        ...
        <textarea
          style={{ ...ST.input, minHeight: "160px", resize: "vertical", lineHeight: "1.65", marginBottom: "12px" }}
          placeholder="Paste or type the client brief here — e.g. We're opening a 4,000 sq ft members club in Mayfair..."
          value={briefText}
          onChange={e => setBriefText(e.target.value)}
        />
        <button
          style={{ ...ST.btn("primary"), width: "100%", justifyContent: "center", opacity: briefing || !briefText.trim() ? 0.55 : 1 }}
      ...
      {briefResult && (
        <div style={{ ...ST.card, borderColor: `${C.gold}44` }}>
          ...
          {briefResult.split('\n').map((line, i) => {
            const isHeading = ["PROJECT SCOPE", "AUDIO CHALLENGES", "RECOMMENDED APPROACH", "COMPLEXITY & FEE RANGE", "NEXT STEPS"].some(h => line.trim().startsWith(h));
            return line.trim() ? (
              <div key={i} style={{
                fontSize: isHeading ? "10px" : "13px",
                fontFamily: isHeading ? "DM Mono, monospace" : "Outfit, sans-serif",
                color: isHeading ? C.goldLight : "#ccc",
                lineHeight: isHeading ? "1" : "1.7",
                letterSpacing: isHeading ? "0.1em" : "0",
                fontWeight: isHeading ? "500" : "400",
                marginBottom: isHeading ? "6px" : "14px",
                marginTop: isHeading && i > 0 ? "16px" : "0",
      ...
      <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, backgroundColor: C.surface, borderTop: `1px solid ${C.border}`, display: "flex" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{
            flex: 1, padding: "10px 4px 13px", background: "none", border: "none", cursor: "pointer",
